...
X_distance = min([bin(int(c)).count("1") for c in pw_rank_arr[N-Kz:]])   # log(dz) of Cz = [N, Kz, dz] (X-type codeword)
Z_distance = n - max([bin(int(c)).count("1") for c in pw_rank_arr[:Kx]]) # log(dx) of Cx = [N, Kx, dx] (Z-type codeword)
